...
class HACamSelectEntity(SelectEntity):
...
class HACamLensSelect(HACamSelectEntity):
    """Lens selection entity for Home Assistant IP Camera."""
...
class HACamModeSelect(HACamSelectEntity):
    """Operating mode selection (Always On vs On-Demand)."""
...
# ORIENTATION SELECT ENTITY
class HACamOrientationSelect(HACamSelectEntity):
    """Orientation selection entity for Home Assistant IP Camera."""
    
    def __init__(self, entry_id, ip_address: str, http_port: int):
        super().__init__(entry_id, ip_address, http_port)
        self._attr_name = "Camera Orientation"
        self._attr_unique_id = f"{entry_id}_orientation"
        self._attr_icon = "mdi:screen-rotation"
        self._attr_options = ["Landscape", "Portrait"]
        self._attr_current_option = "Landscape"
        self._attr_available = True
        ...
        try:
            ...
            async with session.get(f"http://{self._ip}:{self._port}/status", timeout=2) as response:
                if response.status == 200:
                    ...
                    self._attr_current_option = "Portrait" if orientation == "portrait" else "Landscape"
                    self._attr_available = True
                else:
                    self._attr_available = False
        except Exception:
    ...
    async def async_select_option(self, option: str) -> None:
        orientation_param = "portrait" if option == "Portrait" else "landscape"
        try:
            session = async_get_clientsession(self.hass)
            await session.get(f"http://{self._ip}:{self._port}/orientation?type={orientation_param}", timeout=3)
            self._attr_current_option = option
        except Exception:
            pass
...
async def async_setup_entry(hass, entry, async_add_entities):
    ...
    async_add_entities([
        HACamLensSelect(entry.entry_id, data["ip_address"], data["http_port"]),
        HACamModeSelect(entry.entry_id, data["ip_address"], data["http_port"]),
        HACamOrientationSelect(entry.entry_id, data["ip_address"], data["http_port"])
    ])