...
class MSCamEntity(SwitchEntity):
    """Base entity for MS IP Cam."""
...
class FlashlightSwitch(MSCamEntity):
    """Flashlight control for MS IP Cam."""
...
class VideoRecordingSwitch(MSCamEntity):
    def __init__(self, entry_id, ip_address, http_port):
        super().__init__(entry_id, ip_address, http_port)
        self._attr_name = "Video Recording"
    ...
    async def async_update(self):
        try:
            ...
            async with session.get(f"http://{self._ip}:{self._port}/status", timeout=2) as response:
                if response.status == 200:
                    data = await response.json()
                    self._is_on = data.get("is_recording", False)
        except Exception:
            pass
    ...
    async def async_turn_on(self, **kwargs):
        try:
            session = async_get_clientsession(self.hass)
            await session.get(f"http://{self._ip}:{self._port}/record?action=start", timeout=3)
            self._is_on = True
        except Exception:
            pass
    ...
    async def async_turn_off(self, **kwargs):
        try:
            session = async_get_clientsession(self.hass)
            await session.get(f"http://{self._ip}:{self._port}/record?action=stop", timeout=3)
            self._is_on = False
        except Exception:
            pass
...
async def async_setup_entry(hass, entry, async_add_entities):
    ...
    async_add_entities([
        ...
        VideoRecordingSwitch(entry.entry_id, data["ip_address"], data["http_port"])
    ])
