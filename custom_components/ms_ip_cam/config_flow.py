...
class HAIPCameraConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    """Handle config flow for Home Assistant IP Camera."""
    VERSION = 1
                    ...
                    async with session.get(
                        ...
                        if response.status == 200:
                            ...
                            return self.async_create_entry(
                                title=f"Home Assistant IP Camera ({ip_address})", 
                                data=user_input
                            )
                        else:
